...
Portions of it are derived from, or incorporate, work by others. This file
records that provenance and satisfies the attribution conditions those works
carry. **Attribution below is required when redistributing this repository or
...
| | |
|---|---|
| **Work** | Virtual Test Bed (VTB), `htgr/assembly` — prismatic HTGR assembly model |
...
| **License text** | <https://creativecommons.org/licenses/by/4.0/> — full text also included at [`licenses/CC-BY-4.0.txt`](licenses/CC-BY-4.0.txt) |
...
This repository is **not** the VTB model and should not be represented as such.
The following changes were made:

- **Scope.** The VTB material models a *single* prismatic assembly. This
  repository models a *full core* — a configurable hexagonal lattice of
...
## 2. HTGR-SCAPC — single-channel thermal-hydraulics and Brayton cycle solver
...
ENU 4192). It performs the steady-state single-channel solve — axial coolant
enthalpy rise, convective film drop, graphite conduction, and compact/kernel
temperatures — and closes a recuperated Brayton cycle to report net electrical
output.

It is vendored here, rather than referenced as an external dependency, so that
the coupled neutronics/thermal-hydraulics workflow can be run from a single
repository. The upstream project remains the canonical source.
...
OpenMC is a dependency, not a derivative — no OpenMC source is included here.
...
The **code in this repository** was written by Cade Finney, except for
`ThermalHydraulics/nc_htgr.py` as noted in section 2. Design parameters that
appear in `config.py` and `examples/` reflect the team's collective work.
