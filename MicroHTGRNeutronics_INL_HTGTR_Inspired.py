...
params = {
    ...
    "temperature_K": 1173.15              # desired reactor temperature  in Kelvin
}
...
def run_simulation(params, run_dir):
    """Run a single openMC simulation with the given parameters in the specified directory."""
...
if __name__ == "__main__":
    ...
    if is_parametric:
        ...
        for i, val in enumerate(parametric_values):
            caseNum = i + 1
            runName = f"{parametric_param}_Case_{caseNum:0{len(parametric_values)}d}_{val}"
            
            # Create run-specific directory for current value
            run_dir = os.path.join(BASE_DIR, runName)

            print(f"\n{'='*80}")
            print(f"Runing Case {caseNum:0{len(parametric_values)}d}: {parametric_param} = {val}")
            print(f"Run Directory: {run_dir}")
            print(f"{'='*80}\n")

            # Create temporary copy of params and modify the specified parameter
