# ############################################################################
#
#   EXAMPLE CONFIGURATION — CHUDR full-core coupled CS-depletion run
#
#   Reproduces the published CHUDR fuel-cycle analysis: a 10 MWth prismatic
#   HTGR depleted over 3.19 years with a control rod criticality search and a
#   converged neutronics/thermal-hydraulics coupling performed at every
#   depletion timestep.
#
#   This is the configuration behind every figure in the repository README.
#
#   ------------------------------------------------------------------------
#   TO USE
#   ------------------------------------------------------------------------
#       cp examples/chudr_cs_depletion_config.py config.py
#       export OPENMC_CROSS_SECTIONS=/path/to/cross_sections.xml
#       python main_simulation.py
#
#   (Back up your own config.py first — this overwrites it.)
#
#   ------------------------------------------------------------------------
#   WHAT THIS RUN DOES, STEP BY STEP
#   ------------------------------------------------------------------------
#   For each of the 24 timesteps in "depletion_timesteps_days":
#     1. Insert bank 1 fully, then binary-search bank 2 insertion until
#        k_eff = 1.000 +/- "critical_search_k_tol".
#     2. Iterate OpenMC against the single-channel thermal-hydraulics solver
#        (ThermalHydraulics/nc_htgr.py) until both k_eff and the axial heating
#        profile converge, feeding updated coolant/compact/matrix temperatures
#        back into the transport solve.
#     3. Deplete at the converged critical rod position and temperature field.
#
#   Post-processing then produces k_eff and rod position histories, per-zone
#   burnup and peaking, actinide and fission product inventories, B-10 burnout,
#   BeO fast fluence, and net electrical output over the cycle.
#
#   ------------------------------------------------------------------------
#   COST
#   ------------------------------------------------------------------------
#   This is a large calculation. As configured (50,000 particles x 200 batches,
#   24 timesteps, each with a criticality search and a TH coupling loop) it
#   takes on the order of days on a many-core workstation, and writes tens of
#   gigabytes of output.
#
#   To scope it down for a first run, reduce "depletion_timesteps_days" to a
#   few short steps (e.g. [1, 3, 3, 3]) and lower "particles" to 10_000.
#
#   ------------------------------------------------------------------------
#   REQUIRED SETUP
#   ------------------------------------------------------------------------
#   "rpt_radius" below is calibrated for THIS geometry and fuel. If you change
#   the compact radius, packing fraction, enrichment or lattice pitch, re-run
#   study_execution_mode = "RPTCalibration" and update it, or set
#   "use_homogenized_fuel" = False to model TRISO particles explicitly.
#
# ############################################################################
...
params = {
    ...
    "bank_1_insertion": 0.0,                 # Start all-rods-out; the criticality search sets these each step
    "bank_2_insertion": 0.0,                 # Start all-rods-out; the criticality search sets these each step
